...
def translate_responses(df):
    translator = Translator()
    french_responses = df[df.lang == "FR"].po3a
    ...
    for ind, french_response in nonempty_french_responses.iteritems():
        ...
        df.loc[ind, "po3a"] = translator.translate(
...
def clean_corpus(data_df, column_name, wordfixfile=None, occurs_in_morethan=0):
    ...
    tmp_df[clean_name + "_stems_wordvec"] = tmp_df[clean_name + "_stems"].apply(
        lambda x: [dictionary[dictionary.word == word].index.values[0] for word in x]
    )
    strSer=partial(pd.Series,dtype=str)
    if occurs_in_morethan > 0:
        document_occurence = (
            tmp_df[clean_name + "_stems_wordvec"]
        ...
        # relabel and reindex
        tmp_df[clean_name + "_stems_wordvec_keep"] = tmp_df[
            clean_name + "_stems_wordvec"
        ].apply(lambda x: [wordind for wordind in x if wordind in keepword_inds])
        tmp_df[clean_name + "_stems_keep"] = tmp_df[
            clean_name + "_stems_wordvec_keep"
        ].apply(
            lambda x: [
        ...
        dictionary = build_word_dictionary(tmp_df[clean_name + "_stems_keep"])

        tmp_df[clean_name + "_stems_keepwordvec"] = tmp_df[clean_name + "_stems_keep"].apply(
            lambda x: [
                dictionary[dictionary.word == word].index.values[0] for word in x
            ]
        )
    tmp_df.drop(columns=[clean_name + "_stems_wordvec_keep"])
